import "./chunk-UGGHI3ZQ.js";
import {
  createAbsolutePositionFromRelativePosition,
  createRelativePositionFromTypeIndex,
  unexpectedCase
} from "./chunk-4GNZ4PUC.js";
import "./chunk-CQJWVFGP.js";
import {
  Range2 as Range,
...
var createRelativeSelection = (editor2, monacoModel, type) => {
...
var createMonacoSelectionFromRelativeSelection = (editor2, type, relSel, doc) => {
...
var MonacoBinding = class {
  ...
  constructor(ytext, monacoModel, editors = /* @__PURE__ */ new Set(), awareness = null) {
    ...
    this._beforeTransaction = () => {
      this.mux(() => {
        this._savedSelections = /* @__PURE__ */ new Map();
        editors.forEach((editor2) => {
          if (editor2.getModel() === monacoModel) {
            const rsel = createRelativeSelection(editor2, monacoModel, ytext);
            if (rsel !== null) {
              this._savedSelections.set(editor2, rsel);
            }
          }
        });
      });
    ...
    this._rerenderDecorations = () => {
      editors.forEach((editor2) => {
        if (awareness && editor2.getModel() === monacoModel) {
          const currentDecorations = this._decorations.get(editor2) || [];
          ...
          this._decorations.set(editor2, editor2.deltaDecorations(currentDecorations, newDecorations));
        } else {
          this._decorations.delete(editor2);
        }
      });
    };
    this._ytextObserver = (event) => {
      this.mux(() => {
        ...
        this._savedSelections.forEach((rsel, editor2) => {
          const sel = createMonacoSelectionFromRelativeSelection(editor2, ytext, rsel, this.doc);
          if (sel !== null) {
            editor2.setSelection(sel);
          }
        });
      });
      this._rerenderDecorations();
    ...
    if (awareness) {
      editors.forEach((editor2) => {
        editor2.onDidChangeCursorSelection(() => {
          if (editor2.getModel() === monacoModel) {
            const sel = editor2.getSelection();
            if (sel === null) {
              return;
