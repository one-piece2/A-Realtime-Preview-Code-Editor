  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div
        ...
        style={{ backgroundImage: "url('/one-piece-bg-register.jpg')" }}
      />
      ...
      <div className="relative z-10 w-full max-w-md mx-4 py-8">
        ...
        <div className="bg-card/60 backdrop-blur-xl border border-border/30 rounded-2xl p-8 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <Input
                  id="email"
                  type="email"
                  placeholder="navigator@sunny.go"
                  value={formData.email}
                  onChange={(e) => handleChange("email", e.target.value)}
                  className={`pl-11 h-11 bg-secondary/50 border-border/50 text-foreground placeholder:text-muted-foreground/50 focus:border-primary focus:ring-primary/20 transition-all ${errors.email ? "border-accent focus:ring-accent/20" : ""}`}
                  disabled={isLoading}
                />
              </div>
              {errors.email && (
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <Input
                  ...
                  placeholder="设置你的秘密口令"
                  value={formData.password}
                  onChange={(e) => handleChange("password", e.target.value)}
                  className={`pl-11 pr-11 h-11 bg-secondary/50 border-border/50 text-foreground placeholder:text-muted-foreground/50 focus:border-primary focus:ring-primary/20 transition-all ${errors.password ? "border-accent focus:ring-accent/20" : ""}`}
                  disabled={isLoading}
                />
                <button
                  type="button"
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <Input
                  ...
                  className={`pl-11 pr-11 h-11 bg-secondary/50 border-border/50 text-foreground placeholder:text-muted-foreground/50 focus:border-primary focus:ring-primary/20 transition-all ${errors.confirmPassword ? "border-accent focus:ring-accent/20" : ""}`}
                  disabled={isLoading}
                />
                <button
                  type="button"
            ...
            <Button
              type="submit"
              className="w-full h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-base tracking-wide shadow-[0_0_20px_rgba(255,215,0,0.3)] hover:shadow-[0_0_30px_rgba(255,215,0,0.5)] transition-all mt-2"
              disabled={isLoading}
            >
              {isLoading ? (
                <>
          ...
          <Button
            variant="outline"
            className="w-full h-11 bg-secondary/30 border-border/50 text-foreground hover:bg-secondary/50 hover:border-primary/50 transition-all"
        ...
        <p className="text-center text-xs text-foreground/40 mt-5 italic">{'"one-piece！"'}</p>
      </div>
    </div>
  )
}
